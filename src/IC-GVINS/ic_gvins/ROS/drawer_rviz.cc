...
DrawerRviz::DrawerRviz(ros::NodeHandle &nh)
    ...
    ref_pub_            = nh.advertise<nav_msgs::Path>("ref_path", 2);
...
void DrawerRviz::publishOdometry() {
    ...
    odometry.header.stamp            = stamp;
    odometry.header.frame_id         = frame_id_;
    odometry.pose.pose.position.x    = gnss_.blh[0];
    odometry.pose.pose.position.y    = gnss_.blh[1];
    odometry.pose.pose.position.z    = gnss_.blh[2];
    odometry.pose.pose.orientation.x = quaternion.x();
    odometry.pose.pose.orientation.y = quaternion.y();
    odometry.pose.pose.orientation.z = quaternion.z();
    odometry.pose.pose.orientation.w = quaternion.w();
    ...
    ref_pose_stamped.header.stamp    = stamp;
    ref_pose_stamped.header.frame_id = frame_id_;
    ref_pose_stamped.pose            = odometry.pose.pose;

    ref_path_.header.stamp    = stamp;
    ref_path_.header.frame_id = frame_id_;
    ref_path_.poses.push_back(ref_pose_stamped);
    ref_pub_.publish(ref_path_);
}
...
void DrawerRviz::updateRefMap(const GNSS &gnss) {
    gnss_ = gnss;
}