...
Tracking::Tracking(Camera::Ptr camera, Map::Ptr map, Drawer::Ptr drawer, const string &configfile,
    ...
    // std::cout << "block_cols : "<< block_cols_ << "col :" << col << std::endl;
    // std::cout << "block_rows : "<< block_rows_ << "row :" << row << std::endl;
...
TrackState Tracking::track(Frame::Ptr frame) {
    // Tracking
    // std::cout << "tracking function" << std::endl;
    timecost_.restart();
    ...
    if (isinitializing_) {
        ...
        if (frame_ref_ == nullptr) {
            ...
            // std::cout << "before featuredetection" << std::endl;

            featuresDetection(frame_ref_, false);

            // std::cout << "after featuredetection" << std::endl;
        ...
        if (parallax_ref_ < track_min_parallax_) {
            showTracking();
            // std::cout << "track initializing" << std::endl;
...
bool Tracking::trackReferenceFrame() {
    ...
    if (pts2d_ref_.empty()) {
        LOGW << "No new feature in previous frame 1" << Logging::doubleData(frame_cur_->stamp());
        return false;
    }
    ...
    if (pts2d_ref_.empty()) {
        LOGW << "No new feature in previous frame 2" << Logging::doubleData(frame_cur_->stamp());
        return false;
    }
    // std::cout << "frame 2 : " << pts2d_ref_.size() << std::endl;
    // std::cout << "status : " << status.size() << std::endl;
    int temp_count =0;
    for (size_t i=0; i<(status.size()/10); i++){
        for (size_t j=0; j<10; j++){
           temp_count += status_reverse[10*i+j];
        }
        // std::cout<<std::endl;
    }
    // std::cout << "temp counter is : " << temp_count << std::endl;
    ...
    // std::cout << "frame 3 : " << pts2d_ref_.size() << std::endl;
    if (pts2d_ref_.empty()) {
        LOGW << "No new feature in previous frame 3";
    ...
    auto tracking_function = [&](const tbb::blocked_range<int> &range) {
        for (int k = range.begin(); k != range.end(); k++) {
            ...
            if (blocl_track_num > 0) {
                ...
                // std::cout << "col : " << cols << "row : " << rows <<std::endl;
                // std::cout << "col_sta : " << col_sta << "col_end : " << col_end <<std::endl;
                // std::cout << "row_sta : " << row_sta << "row_end : " << row_end <<std::endl;


                // std::cout << "image width(cols) : " << frame->image().size().width << std::endl;
                // std::cout << "image height(rows) : " << frame->image().size().height << std::endl;
