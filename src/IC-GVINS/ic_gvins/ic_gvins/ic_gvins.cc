...
GVINS::GVINS(const string &configfile, const string &outputpath, Drawer::Ptr drawer) {
    ...
    gnss_.blh.setZero();
    last_refgnss_.blh.setZero();
    refgnss_.blh.setZero();
...
bool GVINS::addNewGnss(const GNSS &gnss) {
    ...
    if (integration_config_.origin.isZero()) {
        ...
        if((gnss_.time - last_gnss_.time)>0.1){
    ...
    // std::cout << "gnss_.blh is : " << gnss_.blh << std::endl;
    isgnssready_ = true;

    return true;
}

bool GVINS::addNewrefGnss(const GNSS &refgnss) {
    // 低频观测, 无需加锁

    // 根据GNSS定位更新重力常量
    // Update the ref GNSS
    // if((refgnss_.time - last_gnss_.time)>0.05){
    //     last_refgnss_      = refgnss_;
    // }

    refgnss_        = refgnss;
    refgnss_.blh    = Earth::global2local(integration_config_.origin, refgnss_.blh);
...
bool GVINS::addNewFrame(const Frame::Ptr &frame) {
    if (gvinsstate_ > GVINS_INITIALIZING_INS) {
        ...
        // std::cout << "frame push skipped" << std::endl;
        return true;
    }
}
...
void GVINS::runTracking() {
    ...
    while (!isfinished_) {
        ...
        while (!frame_buffer_.empty()) {
            TimeCost timecost;
            // std::cout << "frame buffe is not empty" << std::endl;
            Pose pose_b_c;
            double td;
            {
...
bool GVINS::gvinsInitialization() {
    ...
    // std::cout << "for gvinsInitialization imu_buff size is : " << imu_buff.size() << std::endl;
